...
//         fetch('/get-guide/languages') // Получаем языки
//             .then(response => {
//                 if (!response.ok) throw new Error('Сетевая ошибка: ' + response.status);
//                 return response.json();
//             })
...
//             'phone',
//             'info',
//             'language' // Обратите внимание на новый порядок
//         ];
//
...
//             if (field === 'language') {
//                 loadLanguages().then(languageSelect => {
//                     cell.innerHTML = '';
//                     cell.appendChild(languageSelect);
//                 });
//             } else {
//                 cell.innerHTML = `<input type="text" class="form-control" value="${data}" />`;
//             }
...
//                         const actionName = this.getAttribute('data-action');
//                         const eventName = this.getAttribute('data-event');
//                         cancelApplication(actionName, eventName);
//                     };
//                 });
//             })
//             .catch(error => alert('Не удалось загрузить заявки.'));
...
//
//     function loadLanguages() {
//         return fetch('/get-guide/languages') // Получаем языки
//             .then(response => {
//                 if (!response.ok) throw new Error('Сетевая ошибка: ' + response.status);
//                 return response.json();
//             })
...
//             const cell = document.getElementById(field);
//             const data = cell.innerText || cell.value; // получаем данные в зависимости от элемента (span или input)
//
//             if (field === 'language') {
//                 loadLanguages().then(languageSelect => {
//                     cell.innerHTML = ''; // очищаем содержимое ячейки
//                     languageSelect.value = data; // устанавливаем текущее значение языка
//                     cell.appendChild(languageSelect);
//                 });
//             } else {
//                 cell.innerHTML = `<input type="text" class="form-control" value="${data}" />`;
...
//     document.getElementById('Save').onclick = function() {
//         const inputs = document.querySelectorAll('tbody input');
//         const selectedLanguage = document.querySelector('#language select') ? document.querySelector('#language select').value : document.getElementById('language').innerText; // Получаем выбранный язык
//
//         const lastName = inputs[0].value;
//         const firstName = inputs[1].value;
//         const secondName = inputs[2].value;
...
//                 const languageSelect = document.createElement('select');
//                 languageSelect.className = 'form-control';
//                 languageSelect.multiple = true; // Позволяет выбирать несколько языков
//                 languages.forEach(language => {
//                     const option = document.createElement('option');
//                     option.value = language.languageId;
//                     option.textContent = language.languageName;
...
//                     cell.innerHTML = ''; // очищаем содержимое ячейки
//                     cell.appendChild(languageSelect);
//                     // Устанавливаем стиль для отображения всех языков
//                     languageSelect.style.width = '100%'; // Устанавливаем ширину
//                     languageSelect.style.height = 'auto'; // Устанавливаем высоту
//                 });
//             } else {
//                 cell.innerHTML = `<input type="text" class="form-control" value="${data}" />`;
//             }
...
//     document.getElementById('Save').onclick = function() {
//         const inputs = document.querySelectorAll('tbody input');
//         const selectedLanguages = Array.from(document.querySelector('#language select').selectedOptions).map(option => option.textContent).join(', '); // Получаем выбранные языки и преобразуем в строку
//
//         const lastName = inputs[0].value;
//         const firstName = inputs[1].value;
//         const secondName = inputs[2].value;
...
//             phone: phone,
//             volunteerInfo: volunteerInfo,
//             language: selectedLanguages // Изменено на строку языков
//         };
//
//         fetch('/volunteer/updateByMail', {
//             method: 'PUT',
